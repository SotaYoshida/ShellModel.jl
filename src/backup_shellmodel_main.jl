...
function main(sntf,target_nuc,num_ev,target_J;save_wav=false,
    ...
    @timeit to "prep. 1bjumps" begin
        ...
        bifs,jumps = prep_pn(lblock,tdims,l_pbit,l_nbit,
...
function prep_pn(lblock::Int64,tdims::IA,
                 ...
                 pbits::Array{Array{Int64,1}},
                 nbits::Array{Array{Int64,1}},
                 Mps::IA,delMs::IA,bVpn,Vpn) where {IA<:Array{Int64,1}}
    #to = TimerOutput()
    maxDeltaM = maximum(delMs)
    bifs = [ [0,0] ]; deleteat!(bifs,1)
    bfs = [ Int64[ ] for i = 1:lblock]
    bis = [ Int64[ ] for i = 1:lblock]
    for i = 1:lblock
        for j = i:lblock
            if Mps[j] - Mps[i] in delMs
                push!(bifs,[i,j]); push!(bfs[i],j); push!(bis[j],i)
            end
        end
    end
    ...
    @inbounds @threads for bi = 1:lblock 
        ...
        for j = 1:length(bfs[bi])
            ...
            bisearch!(delMs,deltaM,Vret)
            Vidx=Vret[1]; bV = bVpn[Vidx]; V = Vpn[Vidx]
            TF=[true]; ret=[1,-1];ridx=[-1,-1,-1]           
            @inbounds for nth = 1:length(Vpn[Vidx])
                V = Vpn[Vidx][nth]
                if V== 0.0; continue;end
                a,b,c,d = bV[nth]
                plist=pid[]; nlist=nid[]
                idim = tdims[bi]; l_Nn_i=length(nbits[bi])
                fdim = tdims[bf]; l_Nn_f=length(nbits[bf])
                @inbounds for (Npi,pPhi) in enumerate(pbits[bi])
                    ...
                    o_f = fdim + (Npf-1)*l_Nn_f
                    push!(plist,pid(Npi,Npf,o_i,o_f,ret[1]==-1,bi==bf))
                end
                if length(plist) == 0 ;continue;end
