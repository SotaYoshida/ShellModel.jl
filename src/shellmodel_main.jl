...
function readsnt(sntf,Anum) # now the code supports .snt(KSHELL) format only.
...
#p_sps,n_sps:Array{Array{Int64,1}}
...
"""
make bit representation of T=1 (proton-proton&neutron-neutron) interactions foe each {m_z}
"""
...
"""
make bit representations of proton/neutron, Slater determinants
"""
...
function main(sntf,target_nuc,num_ev,target_J;save_wav=false,
    ...
    if length(target_J) > 1;
        println("multiple target J is not supported now specify the single J ",
                " or target_J =[] for low-lying states with any J")
        exit()
    end
    ...
    ## many-body configs. for proton/neutron space
